...
# mutual model2 based on vgg19
class UNet_model2(object):

    def __init__(self, model_id, img_height=512, img_width=1024, batch_size=4):
        self.model_id = model_id

        self.project_dir = "/scratch/lxx/segmentation-master"
    ...
    def add_placeholders(self):
        ...
        self.mate_prob_ph = tf.placeholder(tf.float32,
                    shape=[self.batch_size, self.img_height, self.img_width, self.no_of_classes],
                    name="mate_prob_ph")

    def create_feed_dict(self, imgs_batch, onehot_labels_batch=None,mate_prob_batch=None):
        ...
        if mate_prob_batch is not None:
            feed_dict[self.mate_prob_ph] = mate_prob_batch
    ...
    def add_logits(self):
        ...
        self.conv1_1 = conv(self.imgs_ph, 3, 3, 64, 1, 1, padding = 'SAME', name = '2conv1_1') # [~, w, h, 64]
        self.conv1_2 = conv(self.conv1_1, 3, 3, 64, 1, 1, padding = 'SAME', name = '2conv1_2') # [~, w, h, 64]
        self.pool1 = max_pool(self.conv1_2, 2, 2, 2, 2, padding = 'SAME', name = '2pool1') # [~, w/2, h/2, 64]
        ...
        self.conv2_1 = conv(self.pool1, 3, 3, 128, 1, 1, padding = 'SAME', name = '2conv2_1') # [~, w/2, h/2, 128]
        self.conv2_2 = conv(self.conv2_1, 3, 3, 128, 1, 1, padding = 'SAME', name = '2conv2_2') # [~, w/2, h/2, 128]
        self.pool2 = max_pool(self.conv2_2, 2, 2, 2, 2, padding = 'SAME', name ='2pool2') # [~, w/4, h/4, 128]
        ...
        self.conv3_1 = conv(self.pool2, 3, 3, 256, 1, 1, padding = 'SAME', name = '2conv3_1') # [~, w/4, h/4, 256]
        self.conv3_2 = conv(self.conv3_1, 3, 3, 256, 1, 1, padding = 'SAME', name = '2conv3_2') # [~, w/4, h/4, 256]
        self.conv3_3 = conv(self.conv3_2, 3, 3, 256, 1, 1, padding = 'SAME', name = '2conv3_3') # [~, w/4, h/4, 256]
        self.conv3_4 = conv(self.conv3_3, 3, 3, 256, 1, 1, padding = 'SAME', name = '2conv3_4') # [~, w/4, h/4, 256]
        self.pool3 = max_pool(self.conv3_4, 2, 2, 2, 2, padding = 'SAME', name ='2pool3') # [~, w/8, h/8, 256]
        ...
        self.conv4_1 = conv(self.pool3, 3, 3, 512, 1, 1, padding = 'SAME', name = '2conv4_1') # [~, w/8, h/8, 512]
        self.conv4_2 = conv(self.conv4_1, 3, 3, 512, 1, 1, padding = 'SAME', name = '2conv4_2') # [~, w/8, h/8, 512]
        self.conv4_3 = conv(self.conv4_2, 3, 3, 512, 1, 1, padding = 'SAME', name = '2conv4_3') # [~, w/8, h/8, 512]
        self.conv4_4 = conv(self.conv4_3, 3, 3, 512, 1, 1, padding = 'SAME', name = '2conv4_4') # [~, w/4, h/4, 256]
        self.pool4 = max_pool(self.conv4_4, 2, 2, 2, 2, padding = 'SAME', name ='2pool4') # [~, w/16, h/16, 512]
        ...
        self.conv5_1 = conv(self.pool4, 3, 3, 512, 1, 1, padding = 'SAME', name = '2conv5_1') # [~, w/16, h/16, 512]
        self.conv5_2 = conv(self.conv5_1, 3, 3, 512, 1, 1, padding = 'SAME', name = '2conv5_2') # [~, w/16, h/16, 512]
        self.conv5_3 = conv(self.conv5_2, 3, 3, 512, 1, 1, padding = 'SAME', name = '2conv5_3') # [~, w/16, h/16, 512]
        self.conv5_4 = conv(self.conv5_3, 3, 3, 512, 1, 1, padding = 'SAME', name = '2conv5_4') # [~, w/4, h/4, 256]
        self.pool5 = max_pool(self.conv5_4, 2, 2, 2, 2, padding = 'SAME', name = '2pool5') # [~, w/32, h/32, 512]
        ...
        #decoder
        self.deconv1 = deconv(self.pool5, 3, 3, 512, 2, 2, output_shape=[self.batch_size, self.img_height/16, self.img_width/16, 512], padding='SAME', name = '2deconv1') #[~, w/16, h/16, 512]
        self.norm1 = norm_rescale(self.pool4, 512, '2norm1')                                                                                                      #[~, w/16, h/16, 512]
        ...
        self.conv6_1 = conv(self.concat1, 3, 3, 512, 1, 1, padding = 'SAME', name = '2conv6_1') # [~, w/16, h/16, 512]
        self.conv6_2 = conv(self.conv6_1, 3, 3, 256, 1, 1, padding = 'SAME', name = '2conv6_2') # [~, w/16, h/16, 256]
        print "conv6_1 shape: {}".format(self.conv6_1.shape)
        print "conv6_2 shape: {}".format(self.conv6_2.shape)
        
        self.deconv2 = deconv(self.conv6_2, 3, 3, 256, 2, 2, output_shape=[self.batch_size, self.img_height/8, self.img_width/8, 256], padding='SAME', name = '2deconv2') #[~, w/8, h/8, 256]
        self.norm2 = norm_rescale(self.pool3, 256, '2norm2')                                                                                                      #[~, w/8, h/8, 256]
        ...
        self.conv7_1 = conv(self.concat2, 3, 3, 256, 1, 1, padding = 'SAME', name = '2conv7_1') # [~, w/8, h/8, 256]
        self.conv7_2 = conv(self.conv7_1, 3, 3, 128, 1, 1, padding = 'SAME', name = '2conv7_2') # [~, w/8, h/8, 128]
        print "conv7_1 shape: {}".format(self.conv7_1.shape)
        print "conv7_2 shape: {}".format(self.conv7_2.shape)

        self.deconv3 = deconv(self.conv7_2, 3, 3, 128, 2, 2, output_shape=[self.batch_size, self.img_height/4, self.img_width/4, 128], padding='SAME', name = '2deconv3') #[~, w/4, h/4, 128]
        self.norm3 = norm_rescale(self.pool2, 128, '2norm3')                                                                                                      #[~, w/4, h/4, 128]
        ...
        self.conv8_1 = conv(self.concat3, 3, 3, 128, 1, 1, padding = 'SAME', name = '2conv8_1') # [~, w/4, h/4, 128]
        self.conv8_2 = conv(self.conv8_1, 3, 3, 64, 1, 1, padding = 'SAME', name = '2conv8_2') # [~, w/4, h/4, 64]
        print "conv8_1 shape: {}".format(self.conv8_1.shape)
        print "conv8_2 shape: {}".format(self.conv8_2.shape)
        
        self.deconv4 = deconv(self.conv8_2, 3, 3, 64, 2, 2, output_shape=[self.batch_size, self.img_height/2, self.img_width/2, 64], padding='SAME', name = '2deconv4') #[~, w/2, h/2, 64]
        self.norm4 = norm_rescale(self.pool1, 64, '2norm4')                                                                                                     #[~, w/2, h/2, 64]
        ...
        self.conv9_1 = conv(self.concat4, 3, 3, 64, 1, 1, padding = 'SAME', name = '2conv9_1') # [~, w/2, h/2, 64]
        print "conv9_1 shape: {}".format(self.conv9_1.shape)

        self.deconv5 = deconv(self.conv9_1, 3, 3, 64, 2, 2, output_shape=[self.batch_size, self.img_height, self.img_width, 64], padding='SAME', name = '2deconv5') #[~, w, h, 64]
        self.norm5 = norm_rescale(self.conv1_2, 64, '2norm5')  #[~, w, h, 64]
        self.concat5 = tf.concat([self.deconv5, self.norm5], axis=3) #[~, w, h, 128]
        self.conv10_1 = conv(self.concat5, 1, 1, self.no_of_classes, 1, 1, padding = 'SAME', non_linear="NONE", name='2conv10_1')#[~, w, h, out_channels(<64)]
    ...
    def add_loss_op(self):
        ...
        mate_prob_ph_softmax=tf.nn.softmax(self.mate_prob_ph)
        self.loss_mutual=tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(labels=mate_prob_ph_softmax, logits=self.logits))
                    
        self.loss = self.loss_raw + 0.001*self.loss_mutual
